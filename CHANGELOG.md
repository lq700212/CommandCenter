...
> 真机为主题边框"；新建 RealShot 真进程驱动重拍 8 张对话框 + 三 PDF 同步更新。
> 附带一次自动化事故复盘（见"为什么"）。产品代码零改动。

### 改动范围

- **根因（像素级实测）**：客户区像素级一致（登录框同为 380×256、同色），只差 OS 边框：
...
- **管线分工固化**：静态对话框一律真进程实拍（RealShot）；无硬件演不出的动态态
  （主界面 OK/NG）才走 DocShot harness。`docs/images/` 8 张全换，三 PDF 重转。
- **README 配图说明同步"真进程实拍"**。

### 为什么这么改

- 文档图边框与真机对不上，操作员一眼看出"不是同一个软件"，手册信任度打折。
- **自动化事故复盘**：RealShot V1 用 `SendMessage(BM_CLICK)` 点按钮——SendMessage 是同步的，
  按钮一弹模态框发送线程就卡死，现场每个弹窗都得用户手工关。V2 铁律：
  开窗点击一律 `PostMessage` + WaitTitle，发送线程永不阻塞；配套看门狗（只关非目标小弹窗）、
  FAIL 当场截全屏、单实例预检、全英文日志、finally 验退。V2 重跑 8/8 全绿、干净退出、
  零人工干预。

### 优化点

- A/B 方法固定：真机跑稳同手法抓一张，与 harness 图分区 diff（本轮主界面 mean<10 达标）；
  对话框对版后主界面残留文本光栅噪音正常看不见，不追。

## V2.16.7（2026-09-17）文档清理 + README 图文并茂（一并提交 V2.16.6 三份手册）

> 删过时冗余文档 3 项；README 开头换成"新人 30 秒看懂 + 界面导览"，不了解的人
> 看图即懂是什么。V2.16.6 三份培训手册及配图 harness 上次未单独提交，本次一并入库。
> 产品代码零改动。

### 改动范围

- **删除**：`docs/使用说明/`（6 份旧 docx 手册，被 V2.16.6 三份制替代）/
  `docs/CameraId审查报告.md`（V2.13.11 一次性审查过程稿，结论已落地）/
  `docs/相机响应错位修复方案.md`（V2.14.34 过程稿，结论已并入主文档第四部分
  "应答-指令匹配与残留排空"）。协议类文档（PLC 接口/MES 对接/通讯范式）与
  CHANGELOG 历史记录不动。
- **README 图文并茂**：标题区换本地实拍主图（旧外链图下线）+ 三角色手册入口表 +
  "30 秒看懂一轮生产" + "界面导览"三图（空闲/系统设置/点位配置）；原业务流节
  更名"详细版"与新人导览区分深浅。

### 为什么这么改

- docs 下过程稿与旧 word 手册跟新手册内容重叠，新人分不清看哪份；README 原开头
  全是术语和 ASCII 图，外人看不懂软件是干什么的。

### 优化点

- `check_docs.py` 全绿；现 docs 仅剩主文档 + MES/PLC/范式 + 三份手册 + 配图，
  无过程稿、无重复入口。

## V2.16.6（2026-09-17）培训文档三份制：操作员/客户技术/内部手册 + 实拍配图

> 旧 `docs/上位机操作说明书(.md/_EN.md)` 无图说明书退役，换成带真实截图的三份培训手册
> （操作员版 / 客户技术版 / 内部版，各含同名 PDF）。产品代码零改动。

### 改动范围

- **三份新手册（`docs/光阑视界*.md` + 同名 pdf）**：操作员版（开机/生产流程/报警处置/
...
- **实拍 harness（`IrisVision/tools/DocShot/`）**：`DocShot.cs` 独立编译直连真实窗体，
  ...
  Logs 备份还原；`check_docs.py` 一键核验 + `md_to_pdf.py`（Edge 无头打印）沉淀同目录。
- **旧文档删除**：`docs/上位机操作说明书.md`/`_EN.md`（被三份制替代，无保留意义；
  协议类文档永删不得，本次未动）；README 文档中心表同步三份新手册。

### 为什么这么改

- 旧说明书无图：按钮名、状态文字、数字无证据，现场对不上号；三份制按读者分层
  （操作员/客户技术/内部），权限与敏感信息各归其位（客户/操作员版无 dev 账号，
  密码只给初始值并要求首登修改）。

### 优化点

- `check_docs.py` 全绿（无截断/无锚点/图片 11 对 11/三 PDF 均 >50KB/无 html 残留/
  对外版禁词干净）；改了界面重跑 DocShot 即换图；产品代码零改动，不跑回归。
...
之前混淆脚本只产 `bin\Obfuscated\` 目录，每次都要人工再挑文件打 zip、注意排除 Logs，过程琐碎
还容易把运行时文件混进部署包。把"打包"收进脚本后，一条命令从头到尾出可上传 zip，流程可复现、
不依赖人肉操作。

### 优化点

- 一条命令交付"可直接上传的部署包"，路径固定、命名带版本号、内容干净；
- 版本号与 git tag 自动关联，部署包与 release 版本天然对得上。
...
- **`ScanConfig.IsIgnoredScanText` 前缀通配项静默失效（V2.14.30 潜伏）**：名单按英文逗号拆分，
  而注释/文档给的示例 `"ER,*"` 自身含逗号——被拆成 `"ER"` + 孤立 `"*"` 两项后，`"*"` 前缀为空串
  永不命中，现场照文档配置 `"ER,*"` 根本不过滤 `ER,READ,00` 这类状态文本。
  修复（AppConfig.IsIgnoredScanText）：拆分后把孤立的 `"*"` 并回前一项还原完整通配项
  （孤立 `*` 本就无意义、唯一来源就是被拆开的通配项，旧配置零影响）；等价简写 `"ER*"` 同样生效。
...
- **用例层** `tests.ps1` + `TestRunner.cs`：137 条回归用例，覆盖 8 组——① SN/型号 ASCII 寄存器打包
  （V2.15.17 协议编码核心，含截断/补 0/空串/非 ASCII）；② PLC 从站读写往返（真实建站 502 → 写 SN/
  型号/结果 → 读回校验 → 上电清零 → Dispose 安全；502 被占自动降级注入 DataStore 兜底）；
  ③ 配置模型默认值/新旧 json 兼容/ApplyDefaults 兜底（含 V2.15.17 新字段旧 json 缺省回落 13/12）；
  ④ 扫码错误文本过滤（精确/大小写/前缀通配修复点/多分隔符）；⑤ 窗口布局统一模型（自适应形状/
  手填钳位补行/默认铺排空窗口/孤儿映射防御）；⑥ 点位→程序号映射（型号表命中/回退默认/-1 边界）；
  ⑦ 密码哈希锚点 + DPAPI 记住密码往返；⑧ I18n 双语切换。TestRunner 经 Roslyn csc 编译到
  `bin\Debug\cc_test_runner.exe` 运行（依赖 dll/Logs 目录天然正确），跑完自动删除 runner；
  **红线：禁止调 `ConfigStore.Load()/Save()`**（会覆盖开发机现有配置），配置测试只走内存序列化往返。
- **冒烟层** `smoke.ps1`：启动真实 exe 两轮（防"首启正常二启崩"），验存活 + 当日日志含
  "PLC 从站监听已启动"/"上电初始化"关键字。
- 任一层失败非零退出并打印"期望=…实际=…"；**禁止盲改断言变绿**——本次 137 条首轮跑出 6 FAIL，
  甄别为"4 条断言错 + 1 个真 bug（上文修复）+ 1 条样本笔误"，真 bug 修代码、其余修断言。

### 过程中踩的坑（已固化成约定，见 AGENTS.md）
...
> 需求：把扫码枪获取到的序列号（SN）传给 PLC——PLC 侧除了型号还要拿到本件产品的 SN。
> 传递方式与"产品名称（型号字符串）"完全同一套设定：ASCII 编码、无独立握手信号，寄存器地址
> 紧跟产品名称占用的区段往后排（型号区默认 40008~40012 → SN 区从 40013 起）。

### 改动范围

- **协议扩展（`PlcConfig` 新增两个可配置项）**：
  - `scanSerialNumberAddress`（DataStore 索引，**默认 13** = 协议 **40013**）：SN 区起始地址，
    紧随型号区 40008~40012 之后向后扩展；
  - `scanSerialNumberLen`（**默认 12** 个寄存器 = **24 字符**）：现场条码加长时调大即可，
    PLC 读取长度需与本值×2 字符对齐。
- **`PlcService`**：
  - 新增公开方法 `WriteSerialNumber(string serial)`：按与 `WriteProductModel` 完全一致的规则
    写 SN ASCII 区（每寄存器 2 字符、高字节在前、不足补 0x00、PLC 以 0x00 作结束符）；空串=
    整区清 0；超容量截断并记 WARN 日志（防"PLC 读到的 SN 缺尾"无从排查）；
  - 抽公共打包方法 `PackAsciiToRegisters`（V2.14.13 型号打包逻辑原样迁入），型号/SN 共用一份
    实现，杜绝两处各写一套漂移；
  - 新增 `_currentSerial` 缓存：每次成功写入（含清零）刷新；**从站建站成功（EnsureConnected）
    即回写缓存 SN**（与 `_currentModel` 建站即写同一套设定）——断线重建后 DataStore 全 0，
    回写保证 PLC 重连后读到的仍是最近一拍的 SN 状态；热更整体重建 PlcService 时缓存归空
    （新实例不回填旧值，写空比回填安全）；
  - `ResetResultRegisters` 上电初始化顺带把 SN 区整区清 0 + 作废缓存（防断电重启残留旧 SN 被
    当成新件误读，与结果寄存器复位同一防御思路；日志文案追加"+ SN 区"）。
- **`ProductionCoordinator.StepScanChannel` 四处写入点（顺序保证 SN 先于结果落地，PLC 读到
  结果≠0 时 SN 必已就绪）**：
  1. 扫码 OK 分支：先 `WriteSerialNumber(LatestSerialNumber)` 再写结果 1；
  2. 读码失败分支（`_serialErrorSeen`）：`WriteSerialNumber("")` 清区后再写结果 2；
  3. 等 SN 超时分支：同上清区再写结果 2；
  4. **人工补录覆盖分支（V2.14.33 死等期）**：先把补录 SN 写进 SN 区、再把结果由 2 覆盖成 1
     （PLC 读到 1 后读到的必是本件补录序列号）；日志同步注明 SN 已上报。

### 为什么这么改

- **编码/握手复用型号的既有设定**（用户明确要求"和产品名称一样的设定"）：PLC 侧无需新增任何
  等待/确认逻辑，读到 40004=1 后照读字符串即可；上位机侧也只在既有扫码推进路径上加一次写，
  不引入新状态机、不影响三拍握手节拍。
- **失败/超时清零而非保留旧值**：SN 是"每件一换"的数据，不清零会让 PLC 在本件无码时误读上一件
  的 SN；清零后"40004=1 且 SN=全 0"不可能同时出现，语义干净。
- **补录也要落 SN**：现场约定产品必须有 SN（死等补录），若只覆盖结果不写 SN，PLC 拿到的 OK 件
  会缺序列号，追溯断链。

### 优化点

- 打包逻辑收敛单实现（`PackAsciiToRegisters`），后续再加变长 ASCII 寄存器区直接复用；
- 截断告警 + 可配置长度，现场条码变化无需改代码；
- 建站即写缓存让"断线重建丢 SN"窗口归零。
...
- `docs/上位机PLC通信接口定义文档.md`（交付 PLC 工程师）：版本头、总表、分类速查、布局示意图、
  新增 §2.5 SN 编码说明、功能码范围、交互时序、异常处理、PLC 侧配置要求、§7.6 读 SN 示例、尾注日期；
- `README.md`：PLC 节寄存器表加 40013~40024 行 + 关键字段补两配置项 + 行为约定补"SN 随扫码结果走"；
- `AGENTS.md`：PLC 握手协议段补 SN 区定义与五处写入时机，PlcConfig 字段清单同步。

## V2.15.16（2026-08-20）主界面相机≥3台模式精简为纯下拉列表 + 掉线相机自动置顶

> 需求：相机数量 ≥3 台时主界面右上角只显示相机下拉列表——把列标签的相机指示灯
> （"● 相机"总状态标签）删掉；相机中途掉线时下拉列表要实时更新、把掉线的相机排在列表
> 最前面（操作员不点开列表，收起时第一项就是掉线相机，一眼可见哪台断了）；若同时多台
> 掉线则按默认排序规则、掉线的整体排在前面；全部连接时按默认顺序排列。

### 改动内容

- **≥3 台模式删掉"● 相机"总状态标签（`MainForm` 移除 `_lblCamAggregate`）**：标题栏右上角
  相机区只保留 `_cmbCamOverview` 下拉列表一个控件（`_pnlCamOverview` 容器宽度同步改为
  只装下拉框、去掉标签宽与 8px 间距偏移），布局更紧凑；悬停明细 ToolTip 从原总标签改挂到
  下拉框上，鼠标悬停下拉框仍能看每台相机"名字+IP+连/断"明细。
- **下拉项排序映射 `_camOverviewOrder`（新增字段）**：下拉项下标 i → 相机下标。列表按
  `OrderBy(IsConnected).ThenBy(下标)` 稳定排序——**掉线(false)整体排最前、连接(true)按默认
  顺序在后**；多台掉线时掉线内部仍按默认顺序，不会乱序。
- **掉线实时置顶（`RefreshCameraAggregateStatus` 重写）**：相机 `ConnectionChanged` 事件
  （掉线/重连边沿触发）→ 主窗体订阅 → `BeginInvoke` 回 UI 线程 → 重排 `_camOverviewOrder` →
  重建下拉项文本 → `SelectedIndex = 0`（收起时下拉框显示的就是第一项，有掉线即显示掉线那台，
  无需点开）→ 按最长项重算下拉框/容器宽度 → 刷新悬停明细。全部连接时自动回到默认顺序。
- **自绘圆点/文案按排序映射取相机（`CmbCamOverview_DrawItem`）**：项下标与相机下标不再相等，
  绘制圆点颜色与文案必须经 `_camOverviewOrder` 换算，否则掉线相机的红点/名字会张冠李戴。
- **宽度自适应改为遍历所有项取最长**：旧实现只量"最后一项"文本宽，排序后最后一项未必最长，
  统一遍历 `Items` 取最大宽度定下拉框/容器宽。
- **清理与语言切换同步**：`BuildCameraStatusLights` 重建时一并释放 `_lblCamAggregate`
  （已删）与 `_camOverviewOrder`；`ApplyLanguage` 删掉总标签刷新分支，≥3 台下拉文本/排序/明细
  统一走 `RefreshCameraAggregateStatus` 重建（含语言切换后"相机N"等文案刷新）。

### 优化点

- 标题栏相机区只留一个下拉框，≥3 台时右上角更干净、不再有红绿总标签占位；
- 掉线相机自动置顶 + 收起即显示第一项，操作员不点开列表即可发现哪台相机掉线；
- 实时性由既有 `ConnectionChanged` 边沿事件链路保证，掉线/重连即时刷新、无需轮询。

## V2.15.15（2026-08-20）主界面相机指示灯自适应宽度 + 相机名限长 10 截断

> 需求：相机少于 3 台时，每台相机一个独立指示灯与 PLC/扫码枪灯一起排在主界面右上角
> （从右往左一排）。相机名是自定义的（现场英文名可能很长），要求：① 灯宽自适应——名字
> 长时灯也不无限变宽、整体排布紧凑；② 每个指示灯的文本限制长度，超出用 "..." 截断，
> 文本长度限制 10、不超过 10。

### 改动内容

- **确认现状**：相机 ≥3 台时本就聚拢成"总标签 + 相机下拉"（`_lblCamAggregate` +
  `_cmbCamOverview`），不一一显示；≤2 台时每台一个独立灯，与 PLC/扫码枪灯 Dock.Right
  从右往左排成一排（历史行为）。
- **灯宽自适应（`MainForm.BuildCameraStatusLights` 的 ≤2 台分支）**：相机灯宽度由固定 96px
  改为按"● + 名字"整段文本实测宽度（`TextRenderer.MeasureText` + 左右 4px 留白）自适应——
  名字短灯窄、名字长灯宽，不再占满固定 96px 让标题栏右侧灯阵变宽。
- **相机名限长 10 截断（新增 `TruncateIndicatorText`）**：只有相机名能自定义（PLC/扫码枪名固定），
  相机名超 10 字符截断为"前 7 字符 + ..."（最终名字 ≤10），灯的圆点前缀"● "不计入限长。
  **≤2 台独立灯与 ≥3 台相机下拉列表（`CamOverviewLabel`，IP 保留）统一限长**——长英文名不把
  下拉框/灯阵撑得太宽；下拉悬停/明细、独立灯 ToolTip 仍显示完整名。
  **修复"截短没生效"（V2.15.15 补）**：`ApplyLanguage`（语言切换/热更）重设 ≤2 台相机灯文本时
  误用未截断的 `CamDisplayName`，把创建时的截断名顶回完整名——已改为同样走 `TruncateIndicatorText`
  截断；并抽取 `CamLightTipText` 共用方法（构建/热更/语言刷新三处共用），切语言时 ≤2 台灯
...
- 相机灯宽度自适应 + 名字限长后，标题栏右侧灯区紧凑、不随长名字膨胀，长英文名也不挤占
  左侧字段空间（`RelayoutTitleBar` 的 rightDockWidth 按实测宽统计，排布更精确）；
- 悬停补全完整信息，截断不影响现场看完整相机名/IP/状态；
- 仅动 ≤2 台模式相机灯渲染，≥3 台聚合模式与 PLC/扫码枪灯完全不受影响。

## V2.15.14（2026-08-20）窗口/点位配置窗体格子编号随界面语言切换

> 需求：窗口/点位配置对话框（WindowPointForm）矩阵里每个格子顶部的窗口说明标识
> （"窗口1、窗口2、窗口3…"）在英文界面下也要显示英文（"Windows 1、Windows 2、Windows 3…"），
> 而不是一直硬编码中文。**中英文不混排：英文界面显示英文、中文界面显示中文。**

### 改动内容

- **格子顶部编号随语言切换（`Views/WindowPointForm.cs` `RefreshCells`）**：普通窗口 / 禁用窗口 /
  空窗口三处格子的顶部编号标识统一走 `I18n.T` 渲染——中文界面显示 `窗口 N`、英文界面显示
  `Windows N`（英文用复数 Windows，与主界面/其它窗体保持一致）。后缀（已禁用/空窗口（无点位）/
  相机名·点位号）本就按语言渲染，同步一致。
  - 修复点：此前普通窗口的编号行是硬编码中文 `窗口 {i+1}`（英文界面也不随语言），禁用/空窗口
    英文用的是单数 `Window`，统一修正为按语言 + 复数 `Windows`。
- **解析兜底同步**：`ResolveWindowSource` 两处"解析失败兜底显示窗口编号"的文案恢复 `I18n.T`
...
- 语言切换后编号标识同步切换、不混排，中文/英文界面各自完整可读；
- 仅动格子文本渲染，不涉及任何配置/映射/交互逻辑，风险极低。

## V2.15.13（2026-08-20）{年月日} 日期目录改点分隔（2026.08.20）+ 清理快速路径同步

> 需求：存图日期目录由中文格式"2026年08月11日"改为点分隔"2026.08.20"（年.月.日），
> 预览树与最终实际归档路径都要同步。现场按新格式区分每天的照片更直观、排序更清晰。

### 改动内容

- **渲染格式（主链路）**：`ImageStore.RenderTemplate` 的 `{年月日}` 占位符渲染格式由
  `yyyy年MM月dd日` 改为 `yyyy.MM.dd`（如 `2026.08.20`）。该方法是**全链路唯一渲染点**——
  实际归档目录（`ImageStore.SaveImageFilePair`）与预览树（`DirTreeEditForm.RefreshPreview`，
  走同一 `RenderTemplate`）同步生效，无需各层各改。
- **清理快速路径兼容**：`ImageStore.TryParseDirDate` 新增点分隔格式解析
  （`^(\d{4})\.(\d{2})\.(\d{2})$`，如 `2026.08.20`），存图定期清理（`IsDirExpired`）对新格式
  目录仍走 O(1) 快判；**历史格式 "2026年08月11日" 与紧凑 "20260811" 解析保留**——现场已存的
...
- 日期目录名"年.月.日"点分隔：跨系统/跨语言更通用（无中文字符），文件管理器按名称排序即按日期排序；
- 只动 `{年月日}` 一个占位符，`{年}`/`{月}`/`{日}` 单独占位符、文件名模板、其它层级不受影响；
- 清理兼容三格式，历史存量目录与新格式并存期间都不误删。

## V2.15.12（2026-08-20）占位符显示同步英文（DirTreeEditForm + SettingsForm 文件名模板框）

> 需求：英文界面下，图片存储目录结构配置窗体（DirTreeEditForm）的占位符下拉（cmbPlaceholder）、
> 目录层级列表（lstLevels）、文件名规则框（txtFileNameTpl），以及系统设置窗体（SettingsForm）的
> 文件名模板框，仍显示中文占位符（{年月日}/{点位}/{相机}…），外国操作员/客户看不懂。本次把
> 这些显示内容同步成英文占位符。

### 改动内容

- **公共工具类 `Utils/PlaceholderLocalizer.cs`（新增）**：`ToDisplay`（中文→英文显示）/
  `ToStorage`（英文→中文存储）两个静态方法，统一供 DirTreeEditForm 与 SettingsForm 使用，
  禁止各层各写一套。英文占位符映射：`{年月日}`→`{Date}`、`{年}`→`{Year}`、`{月}`→`{Month}`、
  `{日}`→`{Day}`、`{点位}`→`{Station}`、`{相机}`→`{Camera}`、`{时间}`→`{Time}`
  （`{SN}`/`{OKNG}` 本就是拉丁字母，不翻译）。
- **核心契约（显示层翻译、存储层还原）**：配置存储与 `ImageStore.RenderTemplate` 渲染
  【始终用中文占位符】——英文界面只是"给人看"时翻译成英文，保存时统一还原成中文写回配置，
  绝不把英文占位符写进 appconfig.json（否则归档路径会变成字面 `{Date}` 目录的脏配置）。
- **DirTreeEditForm**：`LoadFromConfig` 层级列表/文件名框经 `ToDisplay` 载入；`AddLevel`/
  `AddLevelInsert` 默认值 `{年月日}`/`{SN}` 同步翻译（英文界面新增层级即显示 `{Date}`/`{SN}`）；
  `OnOk` 保存前经 `ToStorage` 还原；`cmbPlaceholder` 下拉项按语言重建；`RefreshPreview` 渲染前
  还原中文再交 `RenderTemplate`（预览树显示真实归档目录名，不会出现字面 `{Date}`）；`{相机}`
  示例名随语言（英文界面显示 `Upper Camera`）；英文 ToolTip 占位符写法同步英文。
...
- 英文界面下目录结构/文件名配置不再出现中文占位符，操作员/客户可读性提升；
- 纯显示层翻译，配置文件与归档目录结构零影响，中英界面切换数据一致、旧配置（中文占位符）无需迁移；
- 占位符翻译收敛到 `PlaceholderLocalizer` 单一实现，两个窗体共用，杜绝"各层各写一套"。

## V2.15.11（2026-08-19）标题栏连接灯彻底去灰（相机/扫码枪/PLC 均无灰色态）

> 需求：主界面右上角连接指示灯不再允许"灰色"状态——启动/未判定也不该灰糊糊地占位，
> 绿=连上、红=断开就是全部语义，灰色相关代码全部删掉，不留旧兼容。

### 改动内容

- **相机灯（≤2 台独立灯）**：`BuildCameraStatusLights` 创建灯时不再写灰色 ForeColor，
  创建后立即按 `_cameras[i].IsConnected` 上色（未连=红 229,72,77、已连=绿 46,158,107），
  连接事件继续刷新。
- **相机灯（≥3 台聚合标签）**：`_lblCamAggregate` 初始灰色 ForeColor 删除；本就创建后立即
  `RefreshCameraAggregateStatus` 上色。
- **扫码枪灯**：`RefreshScannerStatus` 的"没有任何启用扫码枪 → 灰"分支改为**隐藏本灯**
  （`Visible=false`，不占 Dock 空间——`RelayoutTitleBar` 只统计 Visible 的右侧控件；
  有启用枪时恢复显示）。没有枪就没有状态可表示，不拿灰色冒充"无设备"。
- **设计器**：`MainForm.Designer.cs` 中 `lblPlcStatus`/`lblScannerStatus`/`lblCamPlaceholder`
  三处初始灰色 `ForeColor` 行全部删除（PLC 灯构造时即被 `UpdatePlcStatus` 三态上色、扫码枪灯
  构造时被 `RefreshScannerStatus` 聚合上色、占位标签运行时隐藏，灰色初始值纯属死代码）。
- **未动**：PLC 灯三态（红/黄/绿）逻辑与黄色中间态保持原样；`WindowPointForm` 下发灰的绿色
  禁用格子文字是窗口点位语义色，不属于连接灯范围。
...
- **文档同步**：`docs/IrisVision.md`（第一部分标题栏说明 + 第八部分 V2.15.11 版本 + V1.12.6
  ...
  **`docs/使用说明/` 下 4 份 docx**（中文/英文《用户使用说明（操作员手册）》第八节插入"颜色速认/
  ...
  无启用扫码枪时灯隐藏，**"常见问题排查（现场速查）"节同步插入"颜色速认/Reading the colors"
  说明块**，与操作手册措辞一致）全部同步。
- **文档修复（同日）**：V2.15.11 在 docx 中插入的"颜色速认"说明块经 Word 打开核实发现**嵌进了
   第八节的表格第一格**（COM 插入用 `InsertParagraphAfter`+`Next` 时落点在表格锚段尾、`Next` 误入
   表内），现象为说明文字沾在速查表"现象/Symptom"表头格内。已用 OpenXML（直接改 `document.xml`）
   将 4 份 docx（中/英操作手册 + 中/英设置指南）的说明块移出表格成为**独立正文段落**（保留在
   "常见问题排查/速查"节标题之后、表格之前），表格首格恢复"现象/Symptom"，Word 打开校验
   `InTable=False` 且表格数量不变（操作手册 2 张、设置指南 10 张）。
