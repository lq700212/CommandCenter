...
| `CommandCenter/Views/ScannerFailForm.cs` | 扫码枪异常提醒对话框（V2.14.32）：扫码枪读码失败（推 ERROR 等错误文本）时弹窗，提醒检查扫码枪或【人工补录】接手本件；外观对齐 LoginForm（蓝横幅+白面板+【人工补录】蓝主按钮/【稍后处理】白次按钮），回 OK 由 MainForm 调 PromptManualSerial 接手 |
