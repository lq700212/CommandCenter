...
$binDir   = Join-Path $repoRoot "IrisVision\bin\Debug"
...
$exe = Join-Path $binDir "IrisVision.exe"
if (-not (Test-Path -LiteralPath $exe)) { Write-Host "[UITESTS-FAIL] 未找到 IrisVision.exe（先跑 build.ps1）"; exit 1 }
