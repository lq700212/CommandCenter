...
# 依次执行：构建 → 回归用例集 → 进程冒烟；任一层失败立即非零退出。
# 用法：powershell -ExecutionPolicy Bypass -File run-all.ps1 [-SkipBuild] [-SkipSmoke]
param(
    [switch]$SkipBuild,
    [switch]$SkipSmoke
)
...
elseif ($failed) { Write-Host ">>> 用例层已失败，跳过冒烟层" }

$sw.Stop()
Write-Host ""
if ($failed) { Write-Host ("════ 验证未通过，耗时 {0:n1}s ════" -f $sw.Elapsed.TotalSeconds); exit 1 }
Write-Host ("════ 全部验证通过（构建 + 用例 + 冒烟），耗时 {0:n1}s ════" -f $sw.Elapsed.TotalSeconds)
exit 0
