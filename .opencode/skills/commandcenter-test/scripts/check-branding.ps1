# CommandCenter 自动化测试 - 品牌/图标检查层（V2.16.5 新增）
...
# 由 build.ps1 在"产出 exe"之后自动调用；也可单独跑（要求 bin\Debug\CommandCenter.exe 已存在）。
# 退出码：0=通过，1=失败。
...
$icoFile = Join-Path $repoRoot "CommandCenter\Resources\app.ico"
$csproj   = Join-Path $repoRoot "CommandCenter\CommandCenter.csproj"
