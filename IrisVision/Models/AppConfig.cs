...
namespace IrisVision.Models
{
    ...
    public class AppConfig
    {
        ...
        /// 【V2.15.20】本段无设置页界面，只通过配置变量（appconfig.json 的 sn 段）控制，
        /// 对接要点见 docs/MES对接说明.md。
        /// </summary>
    ...
    public class CameraConfig
    {
        ...
        /// 见 DefaultCameras 与 docs/IrisVision.md §2.3/§7），显示在下拉框/状态灯文案里，
        ...
        // 指令均以 CR(0x0D) 终止；T 系列指令含义见 docs/IrisVision.md 第四部分
        ...
        ///   上相机：U171→点位1~17/P000~P012、Z121→点位1~18/P013~P028；
        ///   下相机：U171→点位1~4/P000~P003、Z121→点位1~3/P005~P007。
        ///   （V2.15.21 现场更新上相机映射：U171 点位缩到 1~17、Z121 缩到 1~18，
        ///   程序号按最新定稿逐点位替换，见下方 Table 实参。）
                ...
                new CameraConfig
                {
                    ...
                    ModelStationPrograms = new List<ModelStationPrograms>
                    {
                        new ModelStationPrograms { ModelName = "U171", Programs = Table(
                            (1, 0), (2, 1), (3, 2), (4, 2), (5, 2), (6, 2), (7, 3), (8, 5),
                            (9, 6), (10, 8), (11, 9), (12, 10), (13, 10), (14, 11), (15, 10),
                            (16, 10), (17, 12)) },
                        new ModelStationPrograms { ModelName = "Z121", Programs = Table(
                            (1, 13), (2, 14), (3, 28), (4, 15), (5, 16), (6, 18), (7, 19),
                            (8, 20), (9, 21), (10, 21), (11, 23), (12, 19), (13, 24),
    ...
    ///
    /// 【V2.7 协议（docs/IrisVision.md §5.5）】请求-结果-复位三拍式握手：
    ...
    public class PlcConfig
    {
        ...
        // ─── 寄存器地址映射（V2.7 协议，见 docs/IrisVision.md §5.5）───
        ...
        /// 写入本寄存器（型号序号来自 ModelIndexes 映射，见下方字段与 docs/IrisVision.md §5.5）；
    ...
    ///   根目录 / 年月日(2026.08.20) / SN号 / OK|NG / 相机 / 点位号文件
    /// 可视化配置入口在设置窗体的"配置目录结构…"，把每级目录名或生成规则编辑成列表。
    /// 注：年月日是【一个】目录名（V2.15.12 起点分隔 "2026.08.20"），不是年/月/日三级目录。
    /// </summary>
    public class ImageConfig
    {
        ...
        ///   ["{年月日}","{SN}","{相机}","{OKNG}"] → 根/2026.08.20/SN-0001/上相机/OK/1.jpeg
        /// 说明：
        ///   {年月日} 是一个整体目录名，V2.15.12 起展开成点分隔 "2026.08.20"（不是年/月/日三级）；
        ...
        public string FtpRootDir { get; set; } = @"D:\IrisVision\Images\ftp";
    }
    ...
    public class SecurityConfig
    {
        ...
        //   开发者账号登录后进的是【开发者模式窗体 DeveloperModeForm（原名 DevTestForm）】——
        //   只做通讯触发/读写验证（V2.15.10 起另有"账号管理"区可重置账号密码），
    ...
    /// <summary>
    /// 扫码 SN 序列号去向配置（V2.15.19，V2.15.20 起简化为二选一，JSON 段 `sn`）。
    ///
    /// 【背景】客户提出后续 SN 可能改为"上位机直接传 MES、不再写 PLC 寄存器区（40013 起）"，
    /// 但 MES 对接方案未最终定稿。为两头都不堵死，"SN 到哪去"做成二选一路由：
    /// Mes = SN 走 HTTP 上传（Services/MesService，默认）；Plc = 走既有 PLC 寄存器写入。
    /// 客户 MES 协议定稿后只改 MesService 的报文格式，配置结构与路由架构不再动。
    ///
    /// 【配置方式（V2.15.20，重要）】本段【没有界面入口】——设置页不提供下拉/输入框，
    /// 只通过配置变量（appconfig.json 的 `sn` 段）控制，手改后重启软件生效：
    /// <code>
    /// "sn": { "target": "Mes", "mesUrl": "http://19.87.x.x:8080/api/sn", "mesTimeoutMs": 3000 }
    /// </code>
    /// 对接要点与代码修改位置见 docs/MES对接说明.md。
    ///
    /// 【字段说明】
    ///   - Target：SN 去向目标（"Mes"/"Plc"，见 SerialNumberTargets 常量与判定）；
    ///   - MesUrl：MES 接收地址（完整 http/https URL）；Target 为 Mes 时必须配置，
    ...
    {
        /// <summary>SN 去向目标（V2.15.20 二选一，默认 "Mes"）：'Mes'=SN 后台 HTTP POST 上传给 MES
        /// （不写 PLC SN 寄存器区）；"Plc"=写 PLC SN 寄存器区（协议 40013 起，不传 MES）。
        /// 无界面入口，只改 appconfig.json 的 sn.target（大小写不敏感）；
        /// 空/非法值由 ConfigStore.ApplyDefaults 归一为 "Mes"（见 SerialNumberTargets.Normalize）。</summary>
        public string Target { get; set; } = SerialNumberTargets.Mes;

        /// <summary>MES 接收 SN 的完整 URL（V2.15.19，如 http://19.87.x.x:8080/api/sn）。
        /// 仅 Target 为 "Mes" 时生效；MesService 以 HTTP POST JSON 提交。</summary>
    ...
    /// <summary>
    /// SN 去向目标的常量与判定规则（V2.15.19；V2.15.20 起二选一：Mes / Plc，互斥不再有"都传"）。
    /// 【为什么收敛成静态类】"该目标要不要写 PLC / 要不要传 MES"的判定在协调器（DeliverSerialNumber）、
    /// 测试用例两处都要用——收敛到这里统一维护，禁止各处再各写一套字符串比较
    /// （改目标名/改默认值时漏一处就是隐性 bug）。
    /// 存储值大小写不敏感（json 手写 "plc" 也能识别），但落盘统一用常量规范值。
    /// 【默认方向】默认目标 = Mes（SN 传 MES）——空/非法值一律归一到 Mes；
    /// 要走既有 PLC 寄存器流程需显式把 sn.target 配成 "Plc"。
    /// </summary>
    public static class SerialNumberTargets
    {
        /// <summary>上传 MES（HTTP POST，见 Services/MesService）——默认目标</summary>
        public const string Mes = "Mes";

        /// <summary>写 PLC SN 寄存器区（协议 40013 起，既有流程，不传 MES）</summary>
        public const string Plc = "Plc";

        /// <summary>
        /// 把任意输入归一成规范目标值：空串/ null /非法值一律回落 "Mes"（默认 SN 传 MES；
        /// 曾在 V2.15.19 回落 Plc，V2.15.20 按客户最新口径默认改为 Mes）。
        /// ConfigStore.ApplyDefaults 统一走本方法，json 手改脏值不会让流程崩溃。
        /// </summary>
        public static string Normalize(string target)
        {
            if (string.Equals(target, Plc, StringComparison.OrdinalIgnoreCase)) return Plc;
            return Mes;   // "Mes"（含大小写变体）与一切非法值都归一到规范 Mes（默认 MES）
        }

        /// <summary>该目标下是否要写 PLC SN 寄存器区（Plc 为 true；Mes 为 false）。
        /// 注意：无论目标是什么，PLC 扫码结果 40004 的 1/2 握手照常——SN 去向只影响 SN 区，不影响握手。</summary>
        public static bool WritesPlc(string target) => string.Equals(Normalize(target), Plc, StringComparison.OrdinalIgnoreCase);

        /// <summary>该目标下是否要上传 MES（Mes 为 true；Plc 为 false）。</summary>
