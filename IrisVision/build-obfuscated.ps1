...
#
# 产物：
#   - 混淆目录：项目\IrisVision\bin\Obfuscated\
#     （现场部署时整个目录拷过去即可，含 exe + 两个第三方 dll + config）
#   - 上传包：  bin\IrisVision_{版本号}_obfuscated.zip（本条为 V2.16 起新增）
#
# 用法（本仓库根目录）：
...
# ---- 路径 ----
$ScriptDir   = Split-Path -Parent $MyInvocation.MyCommand.Path   # ...\IrisVision
...
try {
    & $MsBuild ".\IrisVision.csproj" /p:Configuration=Release /p:Platform=AnyCPU /t:Build /nologo /v:m /m
    if ($LASTEXITCODE -ne 0) { throw "Release 构建失败（MSBuild 退出码 $LASTEXITCODE）" }
    if (-not (Test-Path (Join-Path $ReleaseDir "IrisVision.exe"))) {
        throw "构建产物缺失：Release\IrisVision.exe"
    }
...
if (-not (Test-Path (Join-Path $OutDir "IrisVision.exe"))) {
    throw "混淆产物缺失：bin\Obfuscated\IrisVision.exe"
}
...
if (Test-Path (Join-Path $ReleaseDir "IrisVision.exe.config")) {
    Copy-Item (Join-Path $ReleaseDir "IrisVision.exe.config") $OutDir -Force
}

# 冒烟：启动混淆版 exe，5 秒后确认进程还活着（没崩在启动/反序列化阶段）
$p = Start-Process -FilePath (Join-Path $OutDir "IrisVision.exe") -WorkingDirectory $OutDir -PassThru
...
$ZipPath = Join-Path $ScriptDir "bin\IrisVision_${VersionTag}_obfuscated.zip"
if (Test-Path $ZipPath) { Remove-Item $ZipPath -Force }
# 只打固定 5 个发布文件（不复用"限定通配"以免把 Logs 等运行时文件混进包）
$PackFiles = @(
    (Join-Path $OutDir "IrisVision.exe"),
    (Join-Path $OutDir "IrisVision.exe.config"),
...
Write-Host "  上传包内含：IrisVision.exe（已混淆）/ Newtonsoft.Json.dll / NModbus.dll / IrisVision.exe.config"
