...
> 本技能沉淀自现场验证过的两个上位机项目（IrisVision、AgingTestSystem）的成熟做法。
...
- IrisVision（相机+PLC 命令中心）：`Services/KeyenceIV4Camera.cs`、`Services/PlcService.cs`、`Services/ConnectionMonitor.cs`、`Services/ProductionCoordinator.cs`
