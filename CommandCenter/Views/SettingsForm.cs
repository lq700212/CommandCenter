...
namespace CommandCenter.Views
{
    ...
    /// V2.15.19 曾在设置页摆"SN 传往"下拉与"MES 地址"框，V2.15.20 起删除——SN 去向
    /// （二选一 Mes/Plc，默认 Mes）改为纯配置变量控制（appconfig.json 的 sn 段），
    /// 无界面入口，对接说明见 docs/MES对接说明.md。
    /// </summary>
    public partial class SettingsForm : Form
    {
        ...
        private void LoadFromConfig()
        {
            ...
            // V2.15.20：SN 去向（sn 段）不再在设置页加载/保存——纯配置变量控制（appconfig.json），见类注释。
...
// 删除选中：把当前选中的行整行移除；没有选中行则什么都不做
        ...
        private void OnSave(object sender, EventArgs e)
        {
            ...
            // V2.15.20：SN 去向（sn 段）不再在设置页保存——纯配置变量控制（appconfig.json），见类注释。
