    ...
    partial class SettingsForm
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.tip.SetToolTip(this.btnEditPoints,
                "可视化设置每个窗口对应的相机点位与程序（默认\"前上相机后下相机\"铺排）。\r\n点格子选中→\"编辑点位\"（从相机点位表全部点位里选；选到的点位若被别的窗口占用会【自动互换】）；\"交换位置\"可互换任意两窗口（含上/下相机，交换的是\"窗口↔点位\"对应，不改相机点位/程序表）；\"恢复默认\"重置该型号铺排并全部启用。\r\n改动点【确定】才写回（存按型号的 WindowPointMaps），点【取消】/直接关窗体不生效；自适应/非自适应都可编辑。");
