...
#   现场部署/开发机的 CommandCenter.exe（exe 文件名保持不变，
...
#   powershell -ExecutionPolicy Bypass -File ".\CommandCenter\tools\Create-DesktopShortcut.ps1" `
#     -ExePath "E:\部署\CommandCenter.exe"
#
...
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path   # ...\CommandCenter\tools
$ProjDir   = Split-Path -Parent $ScriptDir                     # ...\CommandCenter

# ---- 确定目标 exe ----
if ([string]::IsNullOrWhiteSpace($ExePath)) {
    $candidates = @(
        (Join-Path $ProjDir "bin\Obfuscated\CommandCenter.exe"),  # 现场部署版优先
        (Join-Path $ProjDir "bin\Release\CommandCenter.exe"),
        (Join-Path $ProjDir "bin\Debug\CommandCenter.exe")
    )
...
if ([string]::IsNullOrWhiteSpace($ExePath) -or -not (Test-Path -LiteralPath $ExePath)) {
    throw "找不到 CommandCenter.exe（自动查找 混淆版/Release/Debug 均无结果），请用 -ExePath 显式指定。"
}
